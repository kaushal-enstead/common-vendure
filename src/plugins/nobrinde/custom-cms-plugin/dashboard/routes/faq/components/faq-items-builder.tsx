...
import { Input, RichTextInput } from '@vendure/dashboard';
      ...
      {!collapsed && (
        <CardContent className="p-3 md:p-4 flex flex-col gap-3 space-y-3">
          <FormFieldWrapper
            ...
            render={({ field }) => (
              <Input {...field} value={field.value as string} placeholder="Enter question..." />
            )}
          />

          <FormFieldWrapper
            control={control}
            name={`${formName}.${index}.explanation`}
...
/** Main FAQ items builder with drag & drop */
export function FaqItemsBuilder({ name }: { name: string }) {
  ...
  const updateIndices = () => {
    ...
    currentValues.forEach((item: any, idx: number) => {
      if (item?.index !== idx) {
        ...
        <SortableContext items={fields.map(f => f.id)} strategy={verticalListSortingStrategy}>
          {fields.length === 0 && (
            <div className="text-sm text-muted-foreground border rounded-md p-3">
              <Trans>
                No FAQ items. Click on <b>Add Item</b> to start.
              </Trans>
            </div>
          )}
