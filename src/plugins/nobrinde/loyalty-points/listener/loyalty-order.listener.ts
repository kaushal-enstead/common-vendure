        ...
        try {
          ...
          await this.connection.withTransaction(outerCtx, async ctx => {
            ...
            if (loyaltyCoupon && order.discounts.length) {
              ...
              Logger.debug(`Redeem points for order ${order.code} ${order.customerId}`, loggerCtx);
            }
            ...
            const settings = await this.loyaltyService.getSettingsByChannelId(
              ctx,
              sellerChannel.id.toString(),
            );
            if (!settings) {
