...
import {
  ...
  fetchNobrindeBudgetPage,
} from './nobrinde-budget-sync-utils';

/**
 * Full sync: set SyncTable.lastSyncedAt to null for `nobrinde_budgets` (see task params `tableName`).
 */
export const syncNobrindeBudgetTask = new ScheduledTask({
  id: 'sync-nobrinde-budget-task',
  description: 'Sync nobrinde budgets and budget lines from external database to vendure database',
  schedule: cron => cron.everyDayAt(1),
  async execute({ injector, params, scheduledContext: ctx }) {
    ...
    console.time('syncNobrindeBudgetTask');
    try {
      ...
      while (hasNext) {
        const rows = await fetchNobrindeBudgetPage(syncService, tableConfig, lastSyncedAt, offset);
        if (!rows.length) {
          hasNext = false;
          if (page === 1) {
        ...
        await applyNobrindeBudgetRowsFromRemote(connection, rows, resultObject);
      ...
      console.error('syncNobrindeBudgetTask error', error);
      ...
      console.timeEnd('syncNobrindeBudgetTask');
      return resultObject;
    }
  },
});
