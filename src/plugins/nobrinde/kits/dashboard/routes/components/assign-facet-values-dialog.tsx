...
export function AssignFacetValuesDialog({
  ...
  const handleAssign = () => {
    ...
    const items =
      entityType === 'kits'
        ? (entitiesData as any)?.kits?.items
    ...
    mutate({
      input: items.map((entity: EntityWithFacetValues) => ({
        ...
        facetValueIds: [
          ...new Set([
            ...entity.facetValues.filter((fv: any) => !removedFacetValues.has(fv.id)).map((fv: any) => fv.id),
                  ...
                  <thead className="bg-muted/50">
                    <tr>
                      <th className="text-left p-3 text-sm font-medium">
                        {entityType === 'kits' ? <Trans>Kit</Trans> : <Trans>Variant</Trans>}
                      </th>
                      {entityType === 'variants' && (
                        <th className="text-left p-3 text-sm font-medium">
                          <Trans>SKU</Trans>
                    ...
                    {items.map((entity: EntityWithFacetValues) => {
                      ...
                      return (
                        <tr key={entity.id} className="border-t">
                          ...
                          {entityType === 'variants' && (
                            <td className="p-3 align-top">
                              <div className="text-sm text-muted-foreground">{entity.sku}</div>
                            </td>
                          )}
                          <td className="p-3">
