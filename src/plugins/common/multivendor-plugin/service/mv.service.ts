...
export class MultivendorService {
    ...
    async registerNewSeller(ctx: RequestContext, input: { shopName: string; seller: CreateSellerInput }) {
        ...
        const isUniqueEmail = await this.isUniqueSellerEmail(superAdminCtx, input.seller.emailAddress);
        if (!isUniqueEmail) {
            throw new InternalServerError('Email address already exists');
        }
        ...
        await this.createCommonShippingMethods(superAdminCtx, input.shopName, channel);
            ...
            translations: [
                {
                    ...
                    name: `Pick up in Store for ${shopName}`,
                },
            ],
        });
    ...
    private async isUniqueSellerEmail(ctx: RequestContext, email: string) {
    ...
    private async createSellerChannelRoleAdmin(
        ...
        const channel = await this.channelService.create(ctx, {
            code: `${shopCode}-${Date.now()}`,
            sellerId: seller.id,
            token: `${shopCode}-${Date.now()}-token`,
    ...
    private async createCommonShippingMethods(ctx: RequestContext, shopName: string, sellerChannel: Channel) {
        const defaultChannel = await this.channelService.getDefaultChannel(ctx);
        const courierShippingMethods = await this.connection.getRepository(ctx, ShippingMethod).find({
            where: {
                fulfillmentHandlerCode: In(['courier-fulfillment', 'dpd-fulfillment']),
            },
            relations: ['channels', 'customFields.mode'],
        });

        // Filter out methods that are assigned to multiple channels
        const filteredMethods = courierShippingMethods.filter(m => m.channels.length === 1);
        const shopCode = normalizeString(shopName, '-');

        for (const method of filteredMethods) {
            const shippingMethod = await this.shippingMethodService.create(ctx, {
                code: `${shopCode}-${method.code}`,
                checker: {
                    code: method.checker.code,
                    arguments: method.checker.args,
                },
                calculator: {
                    code: method.calculator.code,
                    arguments: method.calculator.args,
                },
                fulfillmentHandler: method.fulfillmentHandlerCode,
                translations: method.translations.map(t => ({
                    languageCode: defaultChannel.defaultLanguageCode,
                    name: `${shopName} ${t.name}`,
                })),
